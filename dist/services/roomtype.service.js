"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const roomtype_model_1 = __importDefault(require("../models/roomtype.model"));
class RoomTypeService {
    // Create room type
    createRoomType(roomtype) {
        return __awaiter(this, void 0, void 0, function* () {
            return yield roomtype_model_1.default.create(roomtype);
        });
    }
    // Update room type
    updateRoomType(name, roomtype) {
        return __awaiter(this, void 0, void 0, function* () {
            return yield roomtype_model_1.default.findOneAndUpdate(name, roomtype, { new: true });
        });
    }
    // Delete room type
    deleteRoomType(name) {
        return __awaiter(this, void 0, void 0, function* () {
            return yield roomtype_model_1.default.findOneAndDelete(name);
        });
    }
    // Fetch one roomtype
    fetchOne(filter) {
        return __awaiter(this, void 0, void 0, function* () {
            return yield roomtype_model_1.default.findOne(filter);
        });
    }
    // Fetch all roomtypes
    fetch(filter) {
        return __awaiter(this, void 0, void 0, function* () {
            return yield roomtype_model_1.default.find(filter);
        });
    }
}
exports.default = new RoomTypeService();
